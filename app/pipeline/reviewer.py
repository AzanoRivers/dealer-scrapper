    ...
    for p in pages_data:
        ...
        if meaningful:
            ...
            for img in meaningful:
                entry = f"[{img['_role']}] {img['_src']}"
                if img["_alt"]:
                    entry += f' alt="{img["_alt"]}"'
                if img["_w"]:
                    entry += f" w={img['_w']}"
...
async def run_reviewer(job_id: str, activity_event: asyncio.Event) -> bool:
    ...
    if response_schema is None:
        ...
        await job_manager.fail_job(
            job_id,
            "INTERNAL_ERROR",
            "response_schema missing from job options.",
        )
        return False
    ...
    async def _call_llm(messages: list[dict[str, Any]], max_tokens: int) -> dict[str, Any]:
        """
        Llama al LLM, parsea el JSON y retorna un dict listo para usar.

        Intenta el modelo principal primero; activa el de emergencia en cualquiera
        de estos casos:
        - Timeout / respuesta vacía (2 min sin contestar)
        - LLMAuthError (permanente: _use_fallback_only)
        - LLMParseError del cliente HTTP
        - Contenido inválido: el modelo retornó 200 pero no es JSON válido

        Retorna {} si AMBOS modelos no responden (timeout).
        Lanza LLMAuthError / LLMParseError solo si AMBOS modelos fallan.
        """
        nonlocal actual_provider, actual_model, _use_fallback_only

        async def _activate_fallback(reason: str) -> dict[str, Any]:
        ...
        try:
            ...
            if fallback_client is None:
                raise
            return await _activate_fallback("parse error HTTP")
        ...
        # Intentar parsear la respuesta del primario; si es texto basura → fallback
        try:
            return _parse_json_response(raw)
        except LLMParseError:
            if fallback_client is None:
                raise
            return await _activate_fallback("respuesta inválida (JSON no parseable)")

    # 3b. Load image catalogue (optional — generated by image_crawler after extractor)
    src_to_catalogue: dict[str, dict] = {}
    catalogue_path: Path = job_dir / "image_catalogue.json"
    if catalogue_path.exists():
        try:
            catalogue_data: dict[str, Any] = json.loads(
                catalogue_path.read_text(encoding="utf-8")
            )
            src_to_catalogue = {
                img["src"]: img
                for img in catalogue_data.get("images", [])
                if img.get("src")
            }
            logger.debug(
                "Reviewer: loaded image catalogue — %d unique images for job %s",
                len(src_to_catalogue),
                job_id,
            )
        except Exception as exc:
            logger.warning("Reviewer: cannot load image_catalogue.json for job %s: %s", job_id, exc)
    ...
    for i, batch_hashes in enumerate(batches):
        ...
        try:
            chunk: dict[str, Any] = await _call_llm(batch_messages, max_tokens_override)
        except LLMAuthError:
            await job_manager.fail_job(
        ...
        if not chunk:
            await job_manager.fail_job(
                job_id, "LLM_TIMEOUT",
                "Both LLM models timed out (2 min). Please retry.",
                retry_after=300,
            )
            return False
    ...
    if not chunk_summaries:
        ...
        try:
            merged: dict[str, Any] = await _call_llm(merge_messages, max_tokens_override)
        except LLMAuthError:
            await job_manager.fail_job(
                job_id, "LLM_AUTH_ERROR",
                "Invalid API key or no credits (both models failed).",
        ...
        if not merged:
            await job_manager.fail_job(
                job_id, "LLM_TIMEOUT",
                "Both LLM models timed out in consolidation (2 min). Please retry.",
                retry_after=300,
            )
            return False
    ...
    if schema_errors:
        ...
        await job_manager.fail_job(
            ...
            f"LLM result does not match the requested schema. Errors: {errors_summary}",
        )
        return False
